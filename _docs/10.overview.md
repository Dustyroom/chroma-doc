...
## How Chroma Works
Under the hood, Chroma uses custom _shader property drawers_ to draw the gradients and curves in the inspector, along with a custom context menu, where you can generate nice random gradients, invert them, and so on. Our custom drawer quietly bakes the gradients and curves into textures, and passes them to the shader as textures. The shader then reads the textures and uses them as it sees fit.

## What's Included
* X custom property drawers.
* X custom attributes.
* X demo scenes.
* X example shaders.
...
* Any Rendering Pipeline is supported: Built-in, URP, HDRP and even custom ones.
  ...
  * Due to very limited support of Shader Graph in the Built-in Render Pipeline, Chroma works only with the code shaders in the Built-in Render Pipeline. It does not work with Shader Graph shaders in the Built-in Render Pipeline.
