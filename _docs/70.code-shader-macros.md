...
`SAMPLE_CURVE_X` {: .text-align--center}
...
The following example shows how to use the gradients and curves in code. In this simple example a gradient is used to color the object and a curve is used to control the specular highlight. Here is the final result:
...
With one curve we've added lots of control over the specular highlight. We can control the intensity of the highlight, the size of the highlight and the position of the highlight. This is a very powerful way to control the look of the object.
